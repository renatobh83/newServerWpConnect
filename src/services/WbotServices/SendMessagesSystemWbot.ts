...
import type { Message as WbotMessage, Whatsapp } from '@wppconnect-team/wppconnect';
...
const SendMessagesSystemWbot = async (wbot: Session, tenantId: number | string): Promise<void> => {
	const where = {
		fromMe: true,
	...
	for (const message of messages) {
		...
		if (message.quotedMsg) {
			const inCache: CustomWbotMessage | undefined = await GetWbotMessage(ticket, message.quotedMsg.messageId, 200);
			if (inCache) {
